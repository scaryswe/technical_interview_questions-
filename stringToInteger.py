...
class Solution:
    def myAtoi(self, str: str) -> int:
        ...
        # If the first character is a + or -, remove it from the string
        ...
        for char in str:
            ...
            # Update the result by multiplying it by 10 and adding the integer value of the current digit
            res = res * 10 + int(char)
            
            # If the result is greater than or equal to 2^31 - 1 or less than or equal to -2^31, return the appropriate maximum or minimum value
            if res * sign >= 2**31 - 1:
                return 2**31 - 1
            if res * sign <= -2**31:
                return -2**31
